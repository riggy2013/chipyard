...
class SysClock2VCU128PlacedOverlay(val shell: VCU128ShellBasicOverlays, name: String, val designInput: ClockInputDesignInput, val shellInput: ClockInputShellInput)
...
class SysClock2VCU128ShellPlacer(shell: VCU128ShellBasicOverlays, val shellInput: ClockInputShellInput)(implicit val valName: ValName)
  extends ClockInputShellPlacer[VCU128ShellBasicOverlays]
{
    def place(designInput: ClockInputDesignInput) = new SysClock2VCU128PlacedOverlay(shell, valName.name, designInput, shellInput)
}

case object VCU128DDR2Size extends Field[BigInt](0x40000000L * 2) // 2GB
class DDR2VCU128PlacedOverlay(val shell: VCU128FPGATestHarness, name: String, val designInput: DDRDesignInput, val shellInput: DDRShellInput)
  extends DDRPlacedOverlay[XilinxVCU128MIGPads](name, designInput, shellInput)
{
  ...
  shell { InModuleBody {
    require (shell.sys_clock2.get.isDefined, "Use of DDRVCU128Overlay depends on SysClock2VCU128Overlay")
...
class DDR2VCU128ShellPlacer(shell: VCU128FPGATestHarness, val shellInput: DDRShellInput)(implicit val valName: ValName)
  extends DDRShellPlacer[VCU128FPGATestHarness] {
  def place(designInput: DDRDesignInput) = new DDR2VCU128PlacedOverlay(shell, valName.name, designInput, shellInput)
}
