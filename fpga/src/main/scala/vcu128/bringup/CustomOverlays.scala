...
import sifive.fpgashells.devices.xilinx.xilinxvcu128mig.{XilinxVCU128MIGPads, XilinxVCU128MIGParams, XilinxVCU128MIG}

import testchipip.{TSIHostWidgetIO}

import chipyard.fpga.vcu128.{FMCPMap}

/* Connect the I2C to certain FMC pins */
class BringupI2CVCU128PlacedOverlay(val shell: VCU128ShellBasicOverlays, name: String, val designInput: I2CDesignInput, val shellInput: I2CShellInput)
...
class BringupI2CVCU128ShellPlacer(val shell: VCU128ShellBasicOverlays, val shellInput: I2CShellInput)(implicit val valName: ValName)
  extends I2CShellPlacer[VCU128ShellBasicOverlays]
{
  def place(designInput: I2CDesignInput) = new BringupI2CVCU128PlacedOverlay(shell, valName.name, designInput, shellInput)
}

/* Connect the UART to certain FMC pins */
class BringupUARTVCU128PlacedOverlay(val shell: VCU128ShellBasicOverlays, name: String, val designInput: UARTDesignInput, val shellInput: UARTShellInput)
...
class BringupUARTVCU128ShellPlacer(shell: VCU128ShellBasicOverlays, val shellInput: UARTShellInput)(implicit val valName: ValName)
  extends UARTShellPlacer[VCU128ShellBasicOverlays] {
  def place(designInput: UARTDesignInput) = new BringupUARTVCU128PlacedOverlay(shell, valName.name, designInput, shellInput)
}
...
class BringupGPIOVCU128PlacedOverlay(val shell: VCU128ShellBasicOverlays, name: String, val designInput: GPIODesignInput, val shellInput: GPIOShellInput, gpioNames: Seq[String])
...
class BringupGPIOVCU128ShellPlacer(shell: VCU128ShellBasicOverlays, val shellInput: GPIOShellInput, gpioNames: Seq[String])(implicit val valName: ValName)
  extends GPIOShellPlacer[VCU128ShellBasicOverlays] {
  def place(designInput: GPIODesignInput) = new BringupGPIOVCU128PlacedOverlay(shell, valName.name, designInput, shellInput, gpioNames)
}
...
case object TSIHostVCU128DDRSize extends Field[BigInt](0x40000000L * 2) // 2GB
class TSIHostVCU128PlacedOverlay(val shell: BringupVCU128FPGATestHarness, name: String, val designInput: TSIHostDesignInput, val shellInput: TSIHostShellInput)
...
class BringupTSIHostVCU128PlacedOverlay(override val shell: BringupVCU128FPGATestHarness, override val name: String, override val designInput: TSIHostDesignInput, override val shellInput: TSIHostShellInput)
  extends TSIHostVCU128PlacedOverlay(shell, name, designInput, shellInput)
{
...
class BringupTSIHostVCU128ShellPlacer(shell: BringupVCU128FPGATestHarness, val shellInput: TSIHostShellInput)(implicit val valName: ValName)
  extends TSIHostShellPlacer[BringupVCU128FPGATestHarness] {
  def place(designInput: TSIHostDesignInput) = new BringupTSIHostVCU128PlacedOverlay(shell, valName.name, designInput, shellInput)
}
