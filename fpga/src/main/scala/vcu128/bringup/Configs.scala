...
import chipyard.fpga.vcu128.{WithVCU128Tweaks, WithFPGAFrequency, VCU128DDR2Size}
  ...
  case PeripheryTSIHostKey => List(
    TSIHostParams(
      ...
      targetMasterPortParams = MasterPortParams(
        ...
        size = site(VCU128DDR2Size),
        beatBytes = 8, // comes from test chip
        idBits = 4) // comes from VCU128 idBits in XilinxVCU128MIG
      ))
})

class WithBringupVCU128System extends Config((site, here, up) => {
  case BuildSystem => (p: Parameters) => new BringupVCU128DigitalTop()(p) // use the VCU128-extended bringup digital top
})
